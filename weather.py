...
def pltline(days,mint,maxt):
    ...
    plt.plot(days,maxt,color='red',linewidth = 1,marker='.',markerfacecolor='red',markersize=5) 
    plt.plot(days,mint,color='blue',linewidth = 1,marker='.',markerfacecolor='blue',markersize=5)     
...
def pltbar(days,mint,maxt):  
        rcParams['figure.figsize']=6,4
        days=dates.date2num(days)
        min_temp_bar=plt.bar(days-0.2, mint, width=0.4, color='#45B6FE')
        max_temp_bar=plt.bar(days+0.2, maxt, width=0.4, color='#D31A38')   
        plt.xticks(days)
        x_y_axis=plt.gca()
        xaxis_format=dates.DateFormatter('%d/%m')
